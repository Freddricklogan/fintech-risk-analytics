...
export function makeCharts(Chart) {
  ...
  return {
    donut(canvas, labels, values) {
      mount(canvas, {
        type: 'doughnut',
        data: { labels, datasets: [{ data: values, backgroundColor: labels.map((_, i) => PALETTE[i % PALETTE.length]), borderColor: tokens().panel, borderWidth: 2 }] },
